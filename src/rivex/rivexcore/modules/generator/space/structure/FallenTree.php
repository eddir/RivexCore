...
use rivex\rivexcore\utils\BuildingUtils;
...
class FallenTree extends PopulatorObject
{
    ...
    public function canPlaceObject(ChunkManager $level, int $x, int $y, int $z, Random $random)
    {
        ...
        switch ($this->direction) {
            case 0:
            case 1:// Z+
                $return = array_merge(BuildingUtils::fillCallback(new Vector3($x, $y, $z), new Vector3($x, $y, $z + $this->length), function (Vector3 $v3, ChunkManager $level) {
                    if (!isset(FallenTree::$overridable[$level->getBlockIdAt($v3->x, $v3->y, $v3->z)])) {
                        //echo "$v3 is not overwritable (" . $level->getBlockIdAt($v3->x, $v3->y, $v3->z) . ").\n";
                        return false;
                    }
                    return null;
                }, $level), BuildingUtils::fillCallback(new Vector3($x, $y - 1, $z), new Vector3($x, $y - 1, $z + $this->length), function (Vector3 $v3, ChunkManager $level) {
                    if (isset(FallenTree::$overridable[$level->getBlockIdAt($v3->x, $v3->y, $v3->z)])) {
            ...
            case 2:
            case 3: // X+
                $return = array_merge(BuildingUtils::fillCallback(new Vector3($x, $y, $z), new Vector3($x + $this->length, $y, $z), function (Vector3 $v3, ChunkManager $level) {
                    if (!isset(FallenTree::$overridable[$level->getBlockIdAt($v3->x, $v3->y, $v3->z)])) {
                        //echo "$v3 is not overwritable (" . $level->getBlockIdAt($v3->x, $v3->y, $v3->z) . ").\n";
                        return false;
                    }
                    return null;
                }, $level), BuildingUtils::fillCallback(new Vector3($x, $y - 1, $z), new Vector3($x + $this->length, $y - 1, $z), function (Vector3 $v3, ChunkManager $level) {
                    if (isset(FallenTree::$overridable[$level->getBlockIdAt($v3->x, $v3->y, $v3->z)])) {
                        //echo "$v3 is overwritable (" . $level->getBlockIdAt($v3->x, $v3->y, $v3->z) . ").\n";
                        return false;
                    }
