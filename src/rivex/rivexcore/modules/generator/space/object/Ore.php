...
class Ore
{
    ...
    public function __construct(Random $random, OreType $type, $replacement)
    {
        $this->type = $type;
        $this->random = $random;
    ...
    {
        return $level->getBlockIdAt($x, $y, $z) === $this->replacement;
    }
                    ...
                    for ($y = $startY; $y <= $endY; ++$y) {
                        ...
                        if ($y > 0 and ($sizeX + $sizeY) < 1) {
                            for ($z = $startZ; $z <= $endZ; ++$z) {
                                ...
                                $sizeZ *= $sizeZ;
                                if (($sizeX + $sizeY + $sizeZ) < 1 and $level->getBlockIdAt($x, $y, $z) === $this->replacement) {
